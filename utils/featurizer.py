...
def zippy_maker(aind_tuples, top):
    ...
    for ainds in aind_tuples:
        resid = [top.atom(ai).residue.index for ai in ainds] #unique
        resids += [list(resid)]
        reseq = [top.atom(ai).residue.resSeq for ai in ainds] #unique
        resseqs += [list(reseq)]
        resname = [top.atom(ai).residue.name for ai in ainds] #unique
...
def featurize_all(filenames, featurizer, topology, chunk=1000, stride=1):
    ...
    for file in filenames:
        ...
        for t in md.iterload(file, chunk=chunk, stride=stride, **kwargs):
            ...
            x = featurizer()
            feat_name.append(x.describe_features(t))
            x = x.partial_transform(t)
            n_frames = len(x)
            data.append(x)
    ...
    return np.concatenate(data), np.concatenate(indices), np.array(fns), np.concatenate(feat_name)
...
class BinaryContactFeaturizer(ContactFeaturizer):
    ...
    def __init__(self, contacts='all', scheme='closest', ignore_nonprotein=True, cutoff=5):
